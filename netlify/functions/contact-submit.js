...
export default async (request) => {
  ...
  try {
    ...
    // Send email via the edge function (has the Resend API key as a Supabase secret)
    try {
      const emailRes = await fetch(`${SUPABASE_URL}/functions/v1/send-contact-email`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${SUPABASE_ANON_KEY}`,
      ...
      if (!emailRes.ok) {
        ...
        console.error('Edge function email error:', emailRes.status, errText);
      } else {
